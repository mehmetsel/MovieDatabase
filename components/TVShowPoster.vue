<template>
  <div class="rounded overflow-hidden shadow-lg relative">
    <img :src="`https://image.tmdb.org/t/p/w500${show.poster_path}`" alt="TV Show poster" class="w-full" />

    <!-- Favori Butonu -->
    <button
      class="absolute top-2 right-2 bg-white rounded-full p-2 shadow"
      @click="toggleFavouriteShow(show)"
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        class="w-6 h-6"
        :fill="isFavouriteShow(show.id) ? 'yellow' : 'none'"
        viewBox="0 0 24 24"
        stroke="currentColor"
        stroke-width="2"
      >
...
export default {
  ...
  setup() {
    ...
    const toggleFavouriteShow = (show) => {
      favouritesStore.toggleFavouriteShow(show);
    };

    const isFavouriteShow = (showId) => {
      return favouritesStore.isFavouriteShow(showId);
    };
